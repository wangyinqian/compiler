...
class TemplateParser {
    constructor(selector,data){ this.init(selector,data); }
    init(selector,jsonData){
        const ele = this.query(selector),html = ele.outerHTML;
                  
        let data = {};
        
        const newStr = html.replace(/<[\w]+[\s\w="':\-.;{}]*>[{}.\w]*|<\/[\w]+>/g,n=>{
               if(!data.isFor){ return this.repalceExpression(n,"",jsonData,data); }
               else
               { 
                 ...
                 {
                      const list = jsonData[data.target],len = Array.isArray(list) ? list.length : Object.keys(list).length;
